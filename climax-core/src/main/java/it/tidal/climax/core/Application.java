...
public class Application {
    ...
    public static final String APPLICATION_BUILD = "2018-07-26d";
    ...
    private static void disableAll(Config config) {
    ...
    private static void testPlayground(Config config) {
        ...
        if (IS_DAY) {
            ...
            if (Illness.compare(firstFloorIl, salaIl) > 1) {
                ...
                CoolAutomation finalData = operate(config, cadc0, cam0,
                        salaPt, giardinoPt,
                        piano0Data);
                ...
                if (finalData == null) {
                    ...
                    tempData = operate(config, cadc1, cam1,
                            firstFloorPt, studioPt, piano1Data);
                    daikin1StatusPost.setCoolAutomation(tempData);
                    l.info("...managed also P1!");
                } else if (finalData.getStatus() == Status.OFF
                        || finalData.getOpMode() == OpMode.Fan) {
                ...
                CoolAutomation tempData;
                CoolAutomation finalData = operate(config, cadc1, cam1,
                ...
                if (finalData == null) {
                    ...
                    tempData = operate(config, cadc0, cam0,
                    ...
                    l.info("...managed also PT!");
                        ...
                        && finalData.getStatus() == Status.OFF) {
    ...
    private static CoolAutomation operateShutdown(Config config,
        ...
        } else if (prevData.getStatus() == Status.OFF) {
        ...
        finalData = cam.setAll(targetData, targetData.getOpMode(),
                targetData.getFanSpeed(), Status.OFF, 0);
    ...
    private static CoolAutomation operateFan(Config config,
        ...
        if (prevData == null) {
            ...
            l.debug("No previous data in '{}' so we go automatic!", statusPath);
        } else if (prevData.getStatus() == Status.ON
                && prevData.getOpMode() == OpMode.Fan
                && prevData.getFanSpeed() == FanSpeed.Low) {
        ...
        finalData = cam.setAll(targetData, OpMode.Fan,
                FanSpeed.Low, Status.ON, 0);
    ...
    private static CoolAutomation operate(Config config,
            CoolAutomationDeviceConfig cadc,
            CoolAutomationManager cam,
            ...
            if (shouldUseExternalAir) {
                ...
                if (IS_DAY) {
                    ...
                    if (sourcePt <= 31.0) {
                        ...
                        finalData = cam.setAll(targetData, OpMode.Fan,
                                FanSpeed.Med, Status.ON, 0);
                    } else {
                        finalData = cam.setAll(targetData, OpMode.Fan,
                                FanSpeed.High, Status.ON, 0);
                    }
                } else {
        ...
        if (IS_DAY) {
            ...
            if (sourcePt <= 30.5) {
                finalData = cam.setAll(targetData, OpMode.Fan,
                        FanSpeed.Low, Status.OFF, 0);
            } else if (sourcePt <= 31.5) {
                finalData = cam.setAll(targetData, OpMode.Fan,
                        FanSpeed.Low, Status.ON, 0);
            } else if (sourcePt <= 32.5) {
                finalData = cam.setAll(targetData, OpMode.Cool,
                        FanSpeed.Low, Status.ON, 0);
            } else if (sourcePt <= 33.5) {
                finalData = cam.setAll(targetData, OpMode.Cool,
                        FanSpeed.Low, Status.ON, -1);
            } else if (sourcePt <= 34.5) {
                finalData = cam.setAll(targetData, OpMode.Cool,
                        FanSpeed.Low, Status.ON, -2);
            } else if (sourcePt <= 35.5) {
                finalData = cam.setAll(targetData, OpMode.Cool,
                        FanSpeed.Med, Status.ON, -2);
            } else if (sourcePt <= 36.5) {
                finalData = cam.setAll(targetData, OpMode.Cool,
                        FanSpeed.Med, Status.ON, -3);
            } else {
                finalData = cam.setAll(targetData, OpMode.Cool,
                        FanSpeed.High, Status.ON, -3);
            }
        } else {
